...
import { ApprovedPaymentService } from './approved-payment-service';
import { InMemoryPaymentRepository } from '@test/repositories/in-memory-payment-repository';

describe('Approved Payment Service use case', () => {
  it('should be able to approved payment service', async () => {
    ...
    const approvedPaymentService = new ApprovedPaymentService(
      paymentServiceRepository,
      paymentRepository,
    );
    ...
    const { paymentService } = await approvedPaymentService.execute(
