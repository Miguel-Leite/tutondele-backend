...
describe('Update license use case', () => {
  it('should be able to update a license', async () => {
    ...
    const updateLicense = new UpdateLicense(
      licenseRepository,
      packageRepository,
    );

    const responsePackage = await createPackage.execute(makePackage());
    const responseLicense = await createLicense.execute(
      makeLicense({
    ...
    expect(licenseRepository.licenses[0].code).toEqual(
      responseLicense.license.code,
    );
